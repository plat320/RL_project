...
class GameState:
    ...
    def __init__(self):
        ...
        self.target_x = (np.random.random()-0.5)*5
    ...
    def reset(self):
        ...
        # for corridor
        self.target_x = (np.random.random()-0.5)*5 + 4*index_x
        self.target_y = (np.random.random()-0.5)*3
    ...
    def game_step(self, time_step=0.1, linear_x=0.8, angular_z=0.3):
        ...
        normalized_laser = [normalized_laser[int(math.floor(-90.0 + float(x) *(180.0/23.0)))] for x in range(24)]
